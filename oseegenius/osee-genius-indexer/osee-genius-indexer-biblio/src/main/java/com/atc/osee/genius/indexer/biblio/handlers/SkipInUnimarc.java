 ...
 * @since 1.3
 */
public class SkipInUnimarc extends TagHandler
{	
	private static final String END_SKIP_STRING = ">>";
						...
						for (char subfieldName : subfieldNames)
						{
							...
							for (Subfield subfield : subfields)
							{
								if (subfield != null)
								{
									...
									if (data != null && subfield.getCode() == 'a' && data.indexOf(END_SKIP_STRING)!=-1)
									{
										
										data = data.substring(data.indexOf(END_SKIP_STRING)+END_SKIP_STRING.length());
									}
