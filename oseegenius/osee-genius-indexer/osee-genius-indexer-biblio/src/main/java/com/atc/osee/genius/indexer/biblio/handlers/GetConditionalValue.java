...
/**
 * A tag handler that simply extracts a value from a marc record based on a given expression, only if a mandatory tag is present.
 * 
 * value(954abr) 
 ...
 * @since 1.0
 */
public class GetConditionalValue extends TagHandler
{	
		...
		for (String tag : tags)
		{
			...
			} else if (tag.length() > 3)
			{
				...
				{				 
					...
					{
						final String subFieldNamesString = tag.substring(3).split(";")[1];
						final String mandatorySubfield = tag.substring(3).split(";")[0];
						...
						for (VariableField f : fields)	
						{
							...
							if (containsMandatoryTag(field, mandatorySubfield)) {							
												...
												if (data != null && (filter == null || (filter != null && data.indexOf(filter) == -1)))
												{
													...
													if (buffer.length() != 0) 
													{
														switch(subfield.getCode()) {
															case 'v':
							                        			buffer.append(" [Fonte dell'informazione] ");
							                        			break;
															default:
	...
	protected boolean containsMandatoryTag(DataField field, final String mandatorySubfield) {
		if (mandatorySubfield == null) {
			return true;
		}
		List<Subfield> subfields = field.getSubfields();
		for (Subfield f : subfields) {
			for (int i = 0; i < mandatorySubfield.length() ; i++) {
				if (f.getCode() == mandatorySubfield.charAt(i)) {
					return true;
				}
			}			
		}
