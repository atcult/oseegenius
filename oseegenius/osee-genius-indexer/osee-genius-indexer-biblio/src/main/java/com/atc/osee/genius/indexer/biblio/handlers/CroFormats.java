 ...
 * @since 1.0
 */
public class CroFormats extends TagHandler implements IConstants
{
	@Override
			...
			final SolrInputDocument document) 
	{
    	...
    	{
    		case A:
				if (field008 != null && field008.find("^.{23}[s]")) 
				{
					return Format.ELETTRONICO;
				} 
				return Format.CARTACEO;
    		case C:
    		case D:
    		...
    		case M:
    			if (field008 != null && field008.find("^.{26}[j]")) 
    			{
    				return Format.WEBSITE;
    			} else if(field008 != null && field008.find("^.{26}[b]"))
    			{
    				return Format.CPROG;
    			}
    			else if(field008 != null && field008.find("^.{26}[i]"))
    			{
    				return Format.INTERACTIVE;
    			} else
    			{
    				return Format.COMPUTER_FILE;
    			}   
