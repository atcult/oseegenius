 ...
 * And removes all asterisks "*"
 * 
 * @author aguercio
 * @since 1.2
 */
public final class RemoveTrailingPunctuationAstTagHandler extends Decorator
{
	...
	public Object decorate(final String fieldName, final String value) 
	{
		...
		if (value.endsWith(")") || isProtectedSuffix(value)) {
		...
		{
			if ((value.length() - lastIndexOfSpace <= 5))
			{
				return value.replaceAll(Pattern.quote("*"), "");
			}
		...
		return value.trim().replaceAll(PUNCTUATION_PATTERN, "").replaceAll(Pattern.quote("*"), "").trim();
	}
