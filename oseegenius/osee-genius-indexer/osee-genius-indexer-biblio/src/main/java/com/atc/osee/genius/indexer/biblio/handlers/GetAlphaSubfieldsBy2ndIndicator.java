 ...
 * 
 * (246,#;abcd)   abcd are fields to exclude
 * 
 * @author aguercio
 * @since 1.2 * 
 */
public class GetAlphaSubfieldsBy2ndIndicator extends TagHandler 
{
	...
	{
		String [] tagsAndSubfields = tagMappings.split(";");
		String subfieldToExclude = tagsAndSubfields[1];
		String tagMappingsClean = tagsAndSubfields[0];
		String [] tagsAndIndicator = tagMappingsClean.split(",");
...
//                          {
	                        	...
	                        	if (subfields == null || mustBeProcessed(subfield.getCode(), subfields))
	                        	{
	                        		...
	                        		if(!mustBeProcessed(subfield.getCode(), subfieldToExclude.toCharArray())) {			                   
	                        			if (builder.length() > 0) 
	                        			{
	                        				builder.append(' ');
	                        			}          
