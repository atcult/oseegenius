...
/**
 * Removes the leading zeros from a given value.
 * 
 * @author agazzarini
 * @since 1.0
 */
public class RemoveLeadingZeros extends Decorator
{
	@Override
	...
	{
		return (value != null && value.trim().length() > 0) ? value.replaceFirst("^0+(?!$)", "") : null;
	}
}