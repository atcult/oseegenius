...
/**
 * A tag handler that simply extracts a value from a marc record and filter it if not a archive fount
 * 
 * value(954abr) 
 ...
 * @since 1.0
 */
public class GetArchiveFacet extends TagHandler
{	
	...
	protected boolean isArchiveFond(Record record) {
		DataField _013 = (DataField) record.getVariableField("013");
		Subfield a = _013.getSubfield('a');
		Subfield b = _013.getSubfield('b');
		if (a != null && b != null) {
			return ("0".equals(a.getData()) && "FO".equals(b.getData()));
		}
		return false;
	}
