...
/**
 * A tag handler that extracts a value from a UNIMARC record based on a given expression.
 * Unimarc has many special cases where punction ins inside and where not. So the ahndlem is more complex
 * List of exceptions here managed:
 * 1) 7x0 $b, xxxx
 * 
 * @author agazzarini and ztajoli
 * @since 1.2
 */
public class GetCompoundValueUnimarc extends TagHandler
{	
				...
				{				 
					...
					if (fields != null)
					{
						for(VariableField f : fields) {
							...
							if (value.length() != 0)
							{
								value = value.replace(" , ", ", "); //Clean work to manage 70x
								result.add(value);
							}
						}
					}
