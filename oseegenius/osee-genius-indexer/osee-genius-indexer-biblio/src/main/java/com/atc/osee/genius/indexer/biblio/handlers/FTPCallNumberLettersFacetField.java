 ...
 * @since 1.2
 */
public class FTPCallNumberLettersFacetField extends TagHandler implements IConstants
{
	@Override
	...
	{
		Set<String> callnums = getCallNumberFacetField(record, tagMappings);
		...
		if (callnums != null)
		{
			...
			{
				char [] chars = new char[callnum.length()];
				callnum.getChars(0, callnum.length(), chars, 0);
				
				for (int i = 0; i < chars.length; i++) 
				{
					char letterOrDigit = chars[i];
	...
	public Set<String> getCallNumberFacetField(Record record, String fieldSpec)
	{
