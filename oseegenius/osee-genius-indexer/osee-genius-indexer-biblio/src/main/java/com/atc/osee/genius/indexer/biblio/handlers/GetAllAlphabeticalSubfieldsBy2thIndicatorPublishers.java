...
/**
 * Extracts all the alphabetical subfields of a given tag, if the indicator is right. 
 * It adds also punctuation for 264
 * 
 * @author aguercio
 * @since 1.0
 */
public class GetAllAlphabeticalSubfieldsBy2thIndicatorPublishers extends TagHandler 
{
	                    ...
	                    for (Subfield subfield : (List<Subfield>)dataField.getSubfields())
	                    {
	                        if (Character.isLetter(subfield.getCode()))
	                        {
	                        	...
	                        	if (subfields == null || mustBeProcessed(subfield.getCode(), subfields))
	                        	{
	                        		...
	                        		else if (subfield.getCode() == 'a' && aCount > 1) {
	                        			builder.append(" ;");
	                        		}
