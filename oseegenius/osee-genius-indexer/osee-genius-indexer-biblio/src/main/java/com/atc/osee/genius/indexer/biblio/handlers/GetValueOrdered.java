 ...
 * If it doesn't find any value for that subfield, insert dummy value
 * 
 * @author aguercio
 * @since 1.3
 */
public class GetValueOrdered extends TagHandler
{	
	private final static String SEPARATOR = System.getProperty("subfield.separator", " "); 
			...
			final SolrInputDocument document)
	{
		...
		for (String tag : tags)
		{
			...
			} else if (tag.length() > 3)
			{
				...
				{				 
					final List<DataField> fields = (List<DataField>) record.getVariableFields(tagNumber);
					...
					if (fields != null && !fields.isEmpty())
					{
						...
						for (DataField field : fields)
						{
							...
							if (indicatorFilter != null) 
							{
								...
								if(indicatorFilter.endsWith("?")){
									indicatorCheck = indicators.substring(0,1).equals(indicatorFilter.substring(0, 1));
								}
								else {
