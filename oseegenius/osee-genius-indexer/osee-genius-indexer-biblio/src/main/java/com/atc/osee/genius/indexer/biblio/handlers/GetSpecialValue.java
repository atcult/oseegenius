 ...
 * specialValue(999a--4,5,6) piglia solo se il valore del 999a equivale a 4, 5 oppure 6
 * 
 * 
 * @author Alice
 * @since 1.0
 */
public class GetSpecialValue extends TagHandler
{	
	...
	{
		//BUG 5695 hardcoded for now for Antonianum
		final String[] selectedValuesString = tagMappings.split("--");
		final String tagMappingsAfterSplitSelectedVal = selectedValuesString[0];
		
		final String[] selectedValuesOnly = selectedValuesString[1].split(",");
		
		//final String[] selectedValuesOnly = {"44", "14", "15", "45", "46"}; 
		
		final String [] expressionAndFilter = tagMappingsAfterSplitSelectedVal.split(",");
		String filter = null;
					...
					for (VariableField variableField : variableFields)
					{
						...
						try{
							...
							if (toAdd!=null && toAdd.length()>0)
							{
								...
								if(isSelectedValue(selectedValuesOnly, toAdd)){
									result.add(toAdd);
								}
							}
						}catch (Exception e) {
					...
					if (fields != null && !fields.isEmpty())
					{
						...
						for (VariableField f : fields)
						{
							...
							if (value != null && value.length() != 0)
							{
								...
								if(isSelectedValue(selectedValuesOnly, value)){
									result.add(value);
								}
							}
						}
	...
	private boolean isSelectedValue(String[] selectedValues, String currentValue){		
		for (int i = 0; i < selectedValues.length; i ++ ){
			if(selectedValues[i].equals(currentValue)){
				return true;
			}
		}
		return false;
