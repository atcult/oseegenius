...
<!-- <xsl:call-template name="UR"/>	 -->		
	...
	<xsl:template name="AU">
		<xsl:for-each select="marc:datafield[@tag='700']|marc:datafield[@tag='710'][@ind0]|marc:datafield[@tag='710'][@ind1]|marc:datafield[@tag='702']|marc:datafield[@tag='712'][@ind0]|marc:datafield[@tag='712'][@ind1]">
	...
	<xsl:template name="T3">
		<xsl:for-each select="marc:datafield[@tag='410']/marc:subfield[@code='a']|marc:datafield[@tag='411']/marc:subfield[@code='a']|marc:datafield[@tag='225']/marc:subfield[@code='a']">
	...
	<xsl:template name="VL">
		<xsl:for-each select="marc:datafield[@tag='410']/marc:subfield[@code='v']|marc:datafield[@tag='411']/marc:subfield[@code='v']|marc:datafield[@tag='225']/marc:subfield[@code='v']">
	...
	<xsl:template name="SN">
		<xsl:choose>
			<xsl:when test="marc:datafield[@tag='010']">
				<xsl:choose>
					<xsl:when test="marc:datafield[@tag='010']/marc:subfield[@code='a']">
						<xsl:text>&#13;&#10;</xsl:text><xsl:text>SN  - </xsl:text>
						<xsl:variable name="value" select="marc:datafield[@tag='010']/marc:subfield[@code='a']/text()"/>
						<xsl:value-of select="normalize-space($value)" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:if test="marc:datafield[@tag='010']/marc:subfield[@code='z']">
							<xsl:text>&#13;&#10;</xsl:text><xsl:text>SN  - </xsl:text>
							<xsl:variable name="value" select="marc:datafield[@tag='010']/marc:subfield[@code='z']/text()"/>
							<xsl:value-of select="normalize-space($value)" />
						</xsl:if>
					</xsl:otherwise>
				</xsl:choose>
			...
			<xsl:otherwise>
				<xsl:if test="marc:datafield[@tag='011']/marc:subfield[@code='a']">
					<xsl:text>&#13;&#10;</xsl:text><xsl:text>SN  - </xsl:text>
					<xsl:variable name="value" select="marc:datafield[@tag='011']/marc:subfield[@code='a']/text()"/>
					<xsl:value-of select="normalize-space($value)" />
				</xsl:if>	
			</xsl:otherwise>	
		</xsl:choose>
	...
	<xsl:template name="PB">
		<xsl:if test="marc:datafield[@tag='210']/marc:subfield[@code='c']">
			...
			<xsl:variable name="value" select="marc:datafield[@tag='210']/marc:subfield[@code='c']/text()"/>
			<xsl:value-of select="normalize-space(translate($value,',;/:', ''))" />
		</xsl:if>	
	</xsl:template>

	<xsl:template name="CY">
		<xsl:if test="marc:datafield[@tag='210']/marc:subfield[@code='a']">
			<xsl:text>&#13;&#10;</xsl:text><xsl:text>CY  - </xsl:text>		
			<xsl:variable name="place" select="marc:datafield[@tag='210']/marc:subfield[@code='a']/text()"/>
			<xsl:value-of select="normalize-space(translate($place,';/:', ''))" />
		</xsl:if>	
	</xsl:template>
	
	<xsl:template name="PY">
		<xsl:if test="marc:datafield[@tag='210']/marc:subfield[@code='d']">
			<xsl:text>&#13;&#10;</xsl:text><xsl:text>PY  - </xsl:text>	
			<xsl:variable name="value" select="marc:datafield[@tag='210']/marc:subfield[@code='d']/text()"/>
			<xsl:value-of select="normalize-space(translate($value,';/:', ''))" />
		</xsl:if>	
	</xsl:template>	
	...
	<xsl:template name="ET">	
		<xsl:if test="marc:datafield[@tag='205']/marc:subfield[@code='a']">
			...
			<xsl:value-of select="normalize-space(marc:datafield[@tag='205']/marc:subfield[@code='a']/text())" />
		</xsl:if>
	</xsl:template>	
