	...
	<xsl:template match="record">
		...
		<xsl:if test="datafield[@tag=700 or @tag=710]">
			<xsl:call-template name="show_authors">
				...
				<xsl:with-param name="label">Autore:</xsl:with-param>
						...
						<xsl:choose>
							<xsl:when test="$length>500">
								...
								<div class="col-md-5 col-12">
									<a id="showText" class="toExpand" href="#" >Mostra tutto</a>
									<a id="reduceText" class="toReduce" href="#"  style="display: none">Riduci</a>
								</div>
							</xsl:when>
							<xsl:otherwise>
		...
		<xsl:call-template name="showNotes">
            ...
            <xsl:with-param name="label_5_coll">. Coll.: </xsl:with-param>
		...
		<xsl:call-template name="tag_identifiers">
			<xsl:with-param name="tag">012</xsl:with-param><xsl:with-param name="label">Impronta:</xsl:with-param>
		...
		<xsl:call-template name="tag_identifiers">
			<xsl:with-param name="tag">017</xsl:with-param><xsl:with-param name="label">Altri identificativi:</xsl:with-param>
			<xsl:with-param name="trclass">tag017</xsl:with-param><xsl:with-param name="tdclass">td017</xsl:with-param>
		</xsl:call-template>
		
		<xsl:call-template name="tag_identifiers">
			<xsl:with-param name="tag">020</xsl:with-param><xsl:with-param name="label">Numero BNI:</xsl:with-param>
			<xsl:with-param name="trclass">tag020</xsl:with-param><xsl:with-param name="tdclass">td020</xsl:with-param>
		</xsl:call-template>
		
		<xsl:call-template name="tag_identifiers">
			<xsl:with-param name="tag">022</xsl:with-param><xsl:with-param name="label">Num. di pubblicazione governativa:</xsl:with-param>
			<xsl:with-param name="trclass">tag022</xsl:with-param><xsl:with-param name="tdclass">td022</xsl:with-param>
		</xsl:call-template>
		
		<xsl:call-template name="tag_identifiers">
			<xsl:with-param name="tag">071</xsl:with-param><xsl:with-param name="label">Numero editoriale:</xsl:with-param>
        ...
        <xsl:if test="datafield[@tag=921]">
            ...
            <xsl:with-param name="label">Marca editoriale:</xsl:with-param>
        ...
        <xsl:if test="datafield[@tag=701 or @tag=711]">
			<xsl:call-template name="show_authors">
				...
				<xsl:with-param name="label">Altri autori:</xsl:with-param>
		...
		<xsl:if test="datafield[@tag=702 or @tag=712]">
			<xsl:call-template name="show_authors">
				...
				<xsl:with-param name="label">Autori secondari:</xsl:with-param>
		...
		<xsl:if test="datafield[@tag=856]">
			...
			<div class="row attribute-row py-2" id="tag865">
				...
				<div class="col-md-10 col-9 strSizeOnly">
					<xsl:call-template name="tag_856"><xsl:with-param name="label">Risorsa digitale</xsl:with-param></xsl:call-template>
				</div>				
			</div>
			</xsl:if>
			...
			<xsl:if test="datafield[@tag=049]/subfield[@code='a'] = 'TDMAGDIG'">
				...
				<div class="row attribute-row py-2 strSizeOnly" id="tag865">
					<xsl:call-template name="tag_856MGDIG">
						...
						<xsl:with-param name="label1">Copia depositata in BNCF</xsl:with-param>
						<xsl:with-param name="label2">Repository di Ateneo</xsl:with-param>
					</xsl:call-template>
				</div>
				</xsl:for-each>
			</xsl:if>	
		...
		<div class="row py-3" id="generaldata">
			<div class="col-md-7 col-12 strSizeOnly">
				<xsl:call-template name="tag_fmt" />. | Lingua: <xsl:call-template name="tag_101" /> | Paese: <xsl:call-template name="tag_102" /> | BID: <xsl:value-of select="controlfield[@tag=001]" />
			</div>
			<div class="col-md-5 col-12">
				<a class="float-md-right btn button-search  current-button" href="javascript:manipulateQueryString('v','m21')" >Visualizza in formato Unimarc</a>
			</div>
		</div>
			...
			<div class="row" id="searchlinkcol">
				<div class="col-md-12 col-12 tdSearchLinkCol">
					<a>
						...
						<xsl:text>Trova quanto è collegato a questa serie</xsl:text>
					</a>
				</div>
			</div>
		</xsl:if>
	...
	<xsl:template name="tag_101">
		<xsl:if test="datafield[@tag=101]">
			...
			<xsl:for-each select="subfield">
				...
				<xsl:if test="@code='b'">
					<xsl:text> (lingua intermedia)</xsl:text>
				</xsl:if>
				<xsl:if test="@code='c'">
					<xsl:text> (lingua originale)</xsl:text>
				</xsl:if>
				<xsl:choose>
				<xsl:when test="position()=last()">
					<xsl:text>.</xsl:text>
							...
							<xsl:if test="$type='not_aut'">
								...
								<xsl:if test="subfield[@code=5]">
									<xsl:if test="not(subfield[@code=4])">
										<xsl:text> [Esemplare in: </xsl:text>
