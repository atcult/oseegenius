...
function poll()
{
	...
	$.ajax({
	    ...
	    success: function(data) 
	    {
	    	$('#documents').xslt({xml: data, xslUrl: 'xslt/clustered_hit.xsl'});
		    ...
		    pager.innerHTML = "";
		    pager.innerHTML += '<b>'+(start + 1) + ' - ' + (start + num) + ' of ' + merged + '</b>';
		    drawPager (pager);
...
function facet(facetData)
{
	...
	"<div>"+
		"<div class='viewlet_toggable_head'><h3>"+facetName+"</h3></div>"+
