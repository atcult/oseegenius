...
public class PUBL_HDG extends Descriptor implements Serializable {
	...
	/* (non-Javadoc)
	 * @see librisuite.hibernate.Descriptor#getHeadingQueryString()
	 */
	public String getHeadingQueryString(Locale locale)
		throws DataAccessException {
		return new DAOIndexList().getLocalizedIndexByKey("247S", locale)
			+ " = \""
			+ getPlaceSortForm()
			+ ";"
			+ getNameSortForm()
	...
	public String getHeadingNumberSearchIndexKey() {
		/*
		 * Currently, there is no db-key index established for publishers.  Heading
		 * searches are based on the PPN index (place and name sortforms)
		 */
		return null;
	}
