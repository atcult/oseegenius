...
public class DAOUserAccount extends HibernateUtil {
	...
	/**
	 * Metodo che legge la vista con i dati relativi al numero di versione e la servicePack 	
	 * @return
	 * @throws DataAccessException
	 */
	public List getPatchHistory() throws DataAccessException 
	{
		Session s = currentSession();
		List result = null;
		try {
			result = s.find(" from OPEN_SUITE_RELEASE");
		} catch (HibernateException e) {
			logAndWrap(e);
		}
		return result;
