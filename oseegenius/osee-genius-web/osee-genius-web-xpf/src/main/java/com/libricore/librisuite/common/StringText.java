...
public class StringText implements Serializable {
	...
	/**
	 * Return a string with a separator. The codes are used to exclude them
	 * from the application of the separator
	 * 
	 * @param codes
	 * @param separator
	 * @return a string with a separator
	 */
	public String toDisplayStringWithSeparator(String codes, String separator) {
		String returnString = new String();
		Iterator iter = subfieldList.iterator();
		while (iter.hasNext()) {
			Subfield aStringTextSubField = (Subfield) iter.next();
			if(codes.contains(aStringTextSubField.getCode())){
			...
			if (iter.hasNext()) {
				returnString = returnString.concat(separator);
			}
		}
		return returnString;
	}
