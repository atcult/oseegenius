 ...
 * @since 1.0
 */
public class AddNewWishListServlet extends OseeGeniusCommunityServlet
{
			...
			final HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if (visit.isAuthenticated())
		{
			...
			if (isNotNullOrEmptyString(uri) && isNotNullOrEmptyString(name))
			{
				try 
				{
					getLicense(request).getCommunityPlugin().addNewWishList(visit.getAccount().getId(), uri, name);
