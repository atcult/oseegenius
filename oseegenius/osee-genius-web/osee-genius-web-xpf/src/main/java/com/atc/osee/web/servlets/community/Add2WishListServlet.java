 ...
 * @since 1.0
 */
public class Add2WishListServlet extends OseeGeniusCommunityServlet
{
			...
			final HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if (visit.isAuthenticated() && getLicense(request).isCommunityPluginEnabled())
		{
			...
			String wishListId = request.getParameter(HttpParameter.ID);
			
			if (isNotNullOrEmptyString(uri) && isNotNullOrEmptyString(wishListId))
			{
				
				try 
				{
					getLicense(request).getCommunityPlugin().add2WishList(Long.parseLong(wishListId), uri);
