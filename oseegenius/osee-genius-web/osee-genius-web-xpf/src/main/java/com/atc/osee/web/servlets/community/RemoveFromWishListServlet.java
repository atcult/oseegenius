 ...
 * @since 1.0
 */
public class RemoveFromWishListServlet extends OseeGeniusCommunityServlet
{
			...
			final HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if (visit.isAuthenticated() && getLicense(request).isCommunityPluginEnabled())
		{
			...
			String id = request.getParameter(HttpParameter.WISH_LIST_ID);
			
			if (isNotNullOrEmptyString(uri) && isNotNullOrEmptyString(id))
			{
				try 
				{
					getLicense(request).getCommunityPlugin().removeFromWishList(Long.parseLong(id),uri);
